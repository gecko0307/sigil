...
Sigil features a semi-realtime WebSocket server that updates the global state with 0.1s intervals. It has almost zero overhead when no players are logged in. For storing data, NoSQL approach is used: the source of truth is the state in memory, and it is periodically dumped to a JSON file.
